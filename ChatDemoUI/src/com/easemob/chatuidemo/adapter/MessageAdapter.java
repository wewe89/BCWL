		...
		public void handleMessage(android.os.Message message) {
			...
			case HANDLER_MESSAGE_SELECT_LAST:
				if (activity instanceof BaseChatActivity) {
					ListView listView = ((BaseChatActivity)activity).getListView();
		...
		if (message.direct == Direct.SEND) {
			...
			statusView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (message.getType() == Type.TXT)
						activity.startActivityForResult(intent, BaseChatActivity.REQUEST_CODE_TEXT);
			...
			public boolean onLongClick(View v) {
				activity.startActivityForResult(
						(new Intent(activity, ContextMenu.class)).putExtra("position", position).putExtra("type",
								Type.TXT.ordinal()), BaseChatActivity.REQUEST_CODE_CONTEXT_MENU);
				return true;
			}
		});
...
//	pri
	...
	/**vate void handleCallMessage(EMMessage message, ViewHolder holder, final int position) {
	 //		TextMessageBody txtBody = (TextMessageBody) message.getBody();
			...
			public void run() {
				...
				if (message.status == EMMessage.Status.SUCCESS) {
				    ...
				    if(message.getError() == EMError.MESSAGE_SEND_INVALID_CONTENT){
				        ToastUtil.showToastShort(activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_invalid_content));
				    }else if(message.getError() == EMError.MESSAGE_SEND_NOT_IN_THE_GROUP){
				        ToastUtil.showToastShort(activity.getString(R.string.send_fail) + activity.getString(R.string.error_send_not_in_the_group));
				    }else{
						ToastUtil.showToastShort(activity.getString(R.string.send_fail) + activity.getString(R.string.connect_failuer_toast));
				    }
				}
				notifyDataSetChanged();
			}
